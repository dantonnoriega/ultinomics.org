...
Rstudio's Mine Cetinkaya-Rundel had a [recent post about the `highcharter` package](https://www.rstudio.com/rviews/2016/10/19/creating-interactive-plots-with-r-and-highcharts/), a wrapper for the [Highcharts](http://www.highcharts.com) javascripts library that lets you create super sweet interactive charts in R.
...
This post assumes that you have a good handle of the `tidyverse` as well as basic object and list construction. In short, this is not a beginner's tutorial but also no an advanced `R` tutorial.
...
If I want to change the min, max, the line width, and some labeling quicks of the x-axis, then I just look at the API options for `xAxis` and locate the corresponding values. 
...
- Advanced
    - [ ] `hc$x$hc_opts$series` for understanding structure of series in highcharts
...
- Random 
    - [ ] things. highcharter likes as.Date(), but not character values of dates.
